...
IMAGE_HEIGHT = 64
IMAGE_WIDTH = 64
OUTPUT_DIM = IMAGE_HEIGHT * IMAGE_WIDTH * 3 # Number of pixels in each iamge
...
def DCGANGenerator(n_samples, noise=None, dim=DIM, bn=True, nonlinearity=tf.nn.relu):
    ...
    output = tf.reshape(output, [-1, 8*dim, width, height])
    if bn:
...
def DCGANDiscriminator(inputs, dim=DIM, bn=True, nonlinearity=LeakyReLU):
    output = tf.reshape(inputs, [-1, 3, IMAGE_HEIGHT, IMAGE_WIDTH])
    ...
    output = exportable_disc(bn, dim, nonlinearity, output)
...
def exportable_disc(bn, dim, nonlinearity, output):
    ...
    output = tf.reshape(output, [-1, width * height * 8 * dim])
    output = lib.ops.linear.Linear('Discriminator.Output', width * height * 8 * dim, 1, output)

    return tf.identity(output, name="Conv_part_discriminator")
...
def run(args):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
        ...
        all_real_data_conv = tf.placeholder(tf.int32, shape=[args.batch_size, 3, IMAGE_HEIGHT, IMAGE_WIDTH])
        ...
        for device_index, (device, real_data_conv) in enumerate(zip(DEVICES, split_real_data_conv)):
            with tf.device(device):
                ...
                real_data = tf.reshape(2*((tf.cast(real_data_conv, tf.float32)/255.)-.5), [args.batch_size/len(DEVICES), OUTPUT_DIM])
        ...
        def generate_image(iteration):
            ...
            lib.save_images.save_images(samples.reshape((args.batch_size, 3, IMAGE_HEIGHT, IMAGE_WIDTH)), os.path.join(args.log_dir,'samples_{}.png'.format(iteration)))


        # Dataset iterator
        train_gen = lib.small_imagenet.load(args.batch_size, data_dir=args.data_path)
        ...
        lib.save_images.save_images(_x_r.reshape((args.batch_size/N_GPUS, 3, IMAGE_HEIGHT, IMAGE_WIDTH)), os.path.join(args.log_dir,'samples_groundtruth.png'))
        ...
        for iteration in xrange(args.max_steps):
            ...
            if (iteration + 1) % 1000 == 0 or (iteration + 1) == args.max_steps:
                checkpoint_file = os.path.join(args.log_dir, 'wgan-imagenet')
